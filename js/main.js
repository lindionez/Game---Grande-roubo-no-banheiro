...
function draw() {
    ...
    for (let r = 0; r < ROWS; r++) {
        for (let c = 0; c < COLS; c++) {
            ...
            else if (tile === 'R') { ctx.fillStyle = '#b71540'; ctx.fillRect(x + 2, y + 2, TILE_SIZE - 4, TILE_SIZE - 4); ctx.strokeStyle = '#e58e26'; ctx.lineWidth = 2; ctx.strokeRect(x + 4, y + 4, TILE_SIZE - 8, TILE_SIZE - 8); ctx.fillStyle = '#f8c291'; for(let i=0; i<5; i++) { ctx.fillRect(x + 6 + i*6, y + 2, 2, 2); ctx.fillRect(x + 6 + i*6, y + TILE_SIZE - 4, 2, 2); } ctx.lineWidth = 1; }
            else if (tile === 'L') { ctx.fillStyle = '#5c3a21'; ctx.fillRect(x + 2, y + 2, TILE_SIZE - 4, TILE_SIZE - 4); ctx.fillStyle = '#8b5a2b'; ctx.fillRect(x + 4, y + 4, TILE_SIZE - 8, TILE_SIZE - 8); ctx.strokeStyle = '#4a2f10'; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(x + TILE_SIZE / 2, y + 4); ctx.lineTo(x + TILE_SIZE / 2, y + TILE_SIZE - 4); ctx.stroke(); ctx.lineWidth = 1; ctx.fillStyle = '#d4af37'; ctx.fillRect(x + TILE_SIZE / 2 - 6, y + TILE_SIZE / 2 - 4, 3, 8); ctx.fillRect(x + TILE_SIZE / 2 + 3, y + TILE_SIZE / 2 - 4, 3, 8); }
            else if (tile === 'C') { ctx.fillStyle = '#5c4033'; ctx.beginPath(); ctx.arc(x + 18, y + 22, 9, 0, Math.PI * 2); ctx.fill(); ctx.beginPath(); ctx.arc(x + 26, y + 20, 7, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#4a2e11'; ctx.beginPath(); ctx.arc(x + 10, y + 14, 4, 0, Math.PI * 2); ctx.fill(); ctx.beginPath(); ctx.arc(x + 32, y + 26, 3, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#3e2723'; ctx.beginPath(); ctx.arc(x + 15, y + 28, 2, 0, Math.PI * 2); ctx.fill(); ctx.beginPath(); ctx.arc(x + 24, y + 15, 2.5, 0, Math.PI * 2); ctx.fill(); }
